...
import { Search, Users, Music2, Play } from 'lucide-react'
...
export default function ArtistView() {
  ...
  const { setPage, showToast } = usePlayer()
  ...
  function searchTrack(trackTitle, artistName) {
    showToast(`Mencari: ${trackTitle}`)
    // Simulate search by going to search page via parent
    window.__musikuSearch?.(`${artistName} ${trackTitle}`)
  }
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex gap-2">
          <div className="flex-1 relative flex items-center">
            ...
            <input
              ...
              className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-3 py-2.5 pl-8 text-[13.5px] text-white placeholder-white/25 outline-none focus:border-white/20 focus:bg-white/[0.07] transition-all"
            />
          </div>
          <button
            onClick={doSearch}
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-32">
        {error && (
          <p className="text-white/40 text-sm text-center py-12">{error}</p>
        )}
          ...
          <div className="animate-fade-in">
            ...
            <div className="flex flex-col gap-1">
              {artist.topTracks.map((track, i) => (
                <div
                  key={track.deezerId}
                  onClick={() => searchTrack(track.title, artist.name)}
                  className="group flex items-center gap-3 px-3 py-2.5 rounded-xl border border-transparent hover:bg-white/[0.04] hover:border-white/[0.08] cursor-pointer transition-all"
                >
                  <span className="w-5 text-center text-[12px] text-white/25 flex-shrink-0 group-hover:hidden">{i + 1}</span>
                  <Play size={13} className="w-5 text-white/50 flex-shrink-0 hidden group-hover:block" fill="currentColor" />
