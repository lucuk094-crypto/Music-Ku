...
export function PlayerProvider({ children }) {
  ...
  const stateRef    = useRef(state)        // ← dideklarasikan PERTAMA
  const toastTimer  = useRef(null)
  const endedRef    = useRef(null)         // ref untuk handleEnded terbaru

  // Selalu sync stateRef dengan state terbaru
  ...
  useEffect(() => {
    ...
    const onEnded    = () => endedRef.current?.()   // ← selalu panggil versi terbaru
    const onError    = () => {
  ...
  const showToast = useCallback((msg) => {
    ...
    toastTimer.current = setTimeout(
      () => dispatch({ type: 'SET_TOAST', payload: null }), 2800
    )
  }, [])
  ...
  const playSong = useCallback(async (item) => {
    ...
    try {
      ...
      const recent = [
        { videoId: item.videoId, title: data.result.title, artist: data.result.artist, thumbnail: data.result.cover || item.thumbnail || '' },
  ...
  // ── Update endedRef setiap kali playNext/repeatMode berubah ──
  useEffect(() => {
    endedRef.current = () => {
      const s = stateRef.current
      if (s.repeatMode === 2) {
        ...
        case 'ArrowRight':
          ...
          if (audioRef.current?.duration)
            audioRef.current.currentTime = Math.min(
              audioRef.current.duration, audioRef.current.currentTime + 10
            )
          break
        case 'ArrowLeft':
          e.preventDefault()
          if (audioRef.current?.duration)
            audioRef.current.currentTime = Math.max(
              0, audioRef.current.currentTime - 10
            )
          break
        case 'KeyM':
          toggleMute()
